...
/// 构建 deps 前原子捕获的 canonical-history 身份（含 preview generation/mode）。
    /// generation 与 mode 必须来自同一个 `GameStatus::history_session()` 快照，
    /// 避免异步 setup 后将旧 generation 和新 mode 撕裂。
