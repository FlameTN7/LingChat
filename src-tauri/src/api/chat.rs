...
// 拉起新的 AI 回复请求（无用户输入，直接触发对话），可携带当轮多模态图片直发
// （原生识图路径，不写记忆）。这是唯一允许捕获全新 HistorySession 身份的入口。
pub async fn trigger_ai_response(
    ...
    trigger_ai_response_for_session(app, game_status, request_session, transient_image).await
}
...
pub async fn feed_image(app: AppHandle, path: String) -> Result<(), String> {
    ...
    if native_vision {
        ...
        if let Some(image) = transient_image {
            ...
            trigger_ai_response_for_session(app, game_status, request_session, Some(image)).await?;
            return Ok(());
        }
    ...
    events::emit_thinking(&app, false);
    trigger_ai_response_for_session(app, game_status, request_session, None).await
}
